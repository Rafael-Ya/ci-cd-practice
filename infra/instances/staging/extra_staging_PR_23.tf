...
# changes.
#
resource "random_id" "server_PR_23" {
  keepers = {
...
resource "aws_instance" "staging_cicd_demo_PR_23" {
  # Read the AMI id "through" the random_id resource to ensure that
  # both will change together.
  ami                    = random_id.server_PR_23.keepers.ami_id
  instance_type          = "t2.micro"
  vpc_security_group_ids = ["sg-0878895fddd50495b"]
  key_name               = aws_key_pair.staging_key.key_name

  tags = {
    "Name" = "staging_cicd_demo-PR_23"
  }
}

output "staging_dns_PR_23" {
  value = aws_instance.staging_cicd_demo_PR_23.public_dns
}
